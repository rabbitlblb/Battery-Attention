...
from pyod.models.auto_encoder_torch import AutoEncoder, check_array, InnerAutoencoder, check_is_fitted, \
...
def load_dataset(unseen, train=True):
    ...
    if train:
        car_number = [k for k in ind_ood_car_dict['ind_sorted'] if int(k[0]) != unseen and int(k[0]) <= 6 ]
    else:  # test
        car_number = [k for k in all_car_dict.keys() if int(k[0]) == unseen ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unseen', type=int, default=2)

    args = parser.parse_args()


    X_train, y_train = load_dataset(unseen=args.unseen, train=True)
    X_test, y_test = load_dataset(unseen=args.unseen, train=False)
    ...
    if args.method == 'auto_encoder':
        ...
        clf = bug_fixed_AutoEncoder(hidden_neurons=[64, 32, 32, 64], preprocessing=True, batch_size=8192, epochs=100,
        ...
        save_prefix = save_prefix + 'unseen_%d' % args.unseen
