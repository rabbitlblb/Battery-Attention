...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        train_dataset = TimeDataset('../data/new1/train_mulmileage', fc_edge_index,
                                                             mode='train',
                                                             config=cfg, unseen=args.unseen, debug=False)
    ...
    def run(self):
        ...
        df.to_csv('./rec_error/gdn_saved_rec_error_unseen%d_%s.csv' % (args.unseen, time_now))
    ...
    def get_score(self, test_result, val_result):
        ...
        time_now = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time()))
        np.save('test_scores_bsz%d_winlgth%d_winstrd%d_unseen%d_%s.npy' % (
            args.batch, args.slide_win, args.slide_stride, args.unseen, time_now),
                test_scores)
        np.save('normal_scores_bsz%d_winlgth%d_winstrd%d_unseen%d_%s.npy' % (
            args.batch, args.slide_win, args.slide_stride, args.unseen, time_now),
                normal_scores)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unseen', help='unseen', type=int, default=2)
