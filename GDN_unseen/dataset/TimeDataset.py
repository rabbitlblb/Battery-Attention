...
class TimeDataset(Dataset):
    def __init__(self, raw_data_path, edge_index, mode='train', config=None, unseen=2, debug=False,
                 specific_car=None):
        self.debug = debug
    ...
    def process(self, raw_data_path):
        ...
        if self.specific_car != None:
            print('using unseen ', self.unseen)
        _ind_car_num_list_len = len(self.ind_car_num_list)

        if self.mode == 'train':
            car_number = [k for k in self.ind_car_num_list if int(k[0]) != self.unseen]
        else:  # test
            car_number = [k for k in self.all_car_dict.keys() if int(k[0]) != self.unseen]
