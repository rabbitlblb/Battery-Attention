...
class Evaluate:
    ...
    def pltAUC(self, train_result, test_result, method):
        try:
            ...
            with open(f'{self.args.auc_picture_path}/{method}-{best_auc}.pkl', "wb") as file:
                pickle.dump(plts, file)
    ...
    def eval(self, train_feature_path, test_feature_path, name, mixcar=False):
        ...
        #Supervised Data
        cars = list(set([x[1] for x in test_label]))
        tr_cars, te_cars = train_test_split(cars, test_size=0.5)
        #print("Supervised Train cars:", tr_cars)
        #print("Supervised Test cars:", te_cars)
        te_tr_x, te_te_x, te_tr_l, te_te_l = [], [], [], []
        for i in range(len(test_x)):
            if test_label[i][1] in tr_cars:
                te_tr_x += [test_x[i]]
                te_tr_l += [test_label[i]]
            else:
                te_te_x += [test_x[i]]
                te_te_l += [test_label[i]]

        train_x_sv = np.vstack((train_x, te_tr_x))
        train_label_sv = np.vstack((train_label, te_tr_l))
        test_x_sv = np.array(te_te_x)
        test_label_sv = np.array(te_te_l)
        #print("Loading supervised feature is :", train_x_sv.shape)
        #print("Loading supervised label is :", train_label_sv.shape)
        #print("Loading supervised test feature is :", test_x_sv.shape)
        #print("Loading supervised test label is :", test_label_sv.shape)
    ...
    def main(self):
        ...
        self.eval(self.args.tf_feature_path+'_batch', self.args.tf_save_feature_path+'_batch', 'tf_batch', mixcar=True)
        self.eval(self.args.tf_feature_path+'_EV', self.args.tf_save_feature_path+'_EV', 'tf_EV', mixcar=True)
        self.eval(self.args.tf_feature_path+'_type', self.args.tf_save_feature_path+'_type', 'tf_type', mixcar=True)
