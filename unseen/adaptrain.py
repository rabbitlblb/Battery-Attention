...
from model import dynamic_vae, distinct_model, transformer_with_BA
...
class Train_adaption:
    ...
    def __init__(self, args, fold_num=0):
        ...
        self.mkdir(tf_feature_path + '_batch')
        self.mkdir(tf_feature_path + '_EV')
        self.mkdir(tf_feature_path + '_type')
        self.mkdir(current_model_path)
        self.mkdir(result_path)
        self.mkdir(save_feature_path)
        self.mkdir(tf_save_feature_path + '_batch')
        self.mkdir(tf_save_feature_path + '_EV')
        self.mkdir(tf_save_feature_path + '_type')
    ...
    def main(self):
        ...
        for x in self.data_loader_battery_train:
            dataloaders += x
        p_bar = tqdm(total=sum([len(dl) for dl in dataloaders])  * (3 *self.args.extract_epochs + 4), desc='saving', ncols=100, mininterval=1, maxinterval=10, miniters=1)
