...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1, battery_num: int = 1, battery_info_rank: int = 1,
        ...
        self.f1lc = Parameter(torch.empty((d_model, battery_info_rank, battery_num), **factory_kwargs))
        self.f1rc = Parameter(torch.empty((dim_feedforward, battery_info_rank, battery_num), **factory_kwargs))
        self.f2lc = Parameter(torch.empty((dim_feedforward, battery_info_rank, battery_num), **factory_kwargs))
        self.f2rc = Parameter(torch.empty((d_model, battery_info_rank, battery_num), **factory_kwargs))
        zeros_(self.f1lc)
        zeros_(self.f2lc)
        normal_(self.f1rc)
        normal_(self.f2rc)
    ...
    def forward(self, src: Tensor, car: Tensor = None) -> Tensor:
        ...
        if self.norm_first:
            ...
            x = x + self._ff_block(self.norm2(x), car)
        else:
            x = self.norm1(x + self._sa_block(x, car))
            x = self.norm2(x + self._ff_block(x, car))
    ...
    # feed forward block
    def _ff_block(self, x: Tensor, car: Tensor) -> Tensor:
        car = car.unsqueeze(1)
        battery_info_f1 = torch.matmul(torch.matmul(self.f1lc, car).squeeze(2), torch.matmul(self.f1rc,car).squeeze(2).T)
        battery_info_f2 = torch.matmul(torch.matmul(self.f2lc, car).squeeze(2), torch.matmul(self.f2rc,car).squeeze(2).T)
        x = self.dropout(self.activation(self.linear1(x) + torch.matmul(x, battery_info_f1)))
        x = self.linear2(x) + torch.matmul(x, battery_info_f2)
...
class TransformerDecoderLayer(Module):
    ...
    # feed forward block
    def _ff_block(self, x: Tensor, car: Tensor = None) -> Tensor:
        car = car.unsqueeze(1)
        battery_info_f1 = torch.matmul(torch.matmul(self.f1lc, car).squeeze(2), torch.matmul(self.f1rc,car).squeeze(2).T)
        battery_info_f2 = torch.matmul(torch.matmul(self.f2lc, car).squeeze(2), torch.matmul(self.f2rc,car).squeeze(2).T)
        x = self.dropout(self.activation(self.linear1(x) + torch.matmul(x, battery_info_f1)))
