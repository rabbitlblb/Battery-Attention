...
def extract(args, step, dataset, data_loader_battery, model, data_task, feature_path, tf_feature_path, p_bar, noise_scale, variable_length):
    ...
    for dataloader in data_loader_battery:
        for batch in dataloader:
            ...
            optimizer = torch.optim.AdamW([car_numbers], lr=1, weight_decay=1e-2)
            scheduler = CosineAnnealingLR(optimizer, T_max=args['extract_epochs'],
                                        eta_min=args['cosine_factor'])
            
            for epoch in range(args['extract_epochs']):
                batch_ = to_var(batch[0]).float()
                target = data_task.target_filter(batch_)
                seq_lengths = batch[1]['seq_lengths'] if variable_length else batch[0].shape[1]
                log_p, mean, log_v, z, mean_pred = model(batch_, encoder_filter=data_task.encoder_filter,
            ...
            log_p, mean, log_v, z, mean_pred = model(batch_, encoder_filter=data_task.encoder_filter,
                                                                ...
                                                                noise_scale=noise_scale)
            target = data_task.target_filter(batch_)
            save_features_info(tf_feature_path+"_batch", batch, iteration, log_p, mean, target)
            p_bar.update(1)
            iteration += 1
    ...
    for dataloader in data_loader_battery:
        ...
        #print('')
        #print(car_numbers)
        #print(torch.nn.Softmax()(car_numbers))
        del optimizer, scheduler
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                log_p, mean, log_v, z, mean_pred = model(batch_, encoder_filter=data_task.encoder_filter,
                                                                ...
                                                                noise_scale=noise_scale)
                target = data_task.target_filter(batch_)
                save_features_info(tf_feature_path+"_EV", batch, iteration, log_p, mean, target)
                p_bar.update(1)
                iteration += 1
    ...
    car_numbers = to_var(car_numbers)
    car_numbers.requires_grad = True
    optimizer = torch.optim.AdamW([car_numbers], lr=args["learning_rate"] * 10000, weight_decay=1e-6)
    scheduler = CosineAnnealingLR(optimizer, T_max=args['extract_epochs'],
                                  eta_min=args['cosine_factor'] * args['learning_rate'] * 10000)
    for p in model.parameters():
        p.require_grad = False
        ...
        for dataloader in data_loader_battery:
            for batch in dataloader:
                ...
                log_p, mean, log_v, z, mean_pred = model(batch_, encoder_filter=data_task.encoder_filter,
                                                            ...
                                                            noise_scale=noise_scale)
                target = data_task.target_filter(batch_)
                save_features_info(tf_feature_path+"_type", batch, iteration, log_p, mean, target)
                p_bar.update(1)
                iteration += 1
...
class Extraction:
    ...
    def main(self):
        ...
        def cmkdir(dir):
            ...
            os.mkdir(dir)
        cmkdir(model_params["save_feature_path"])
        cmkdir(model_params["tf_save_feature_path"]+'_batch')
        cmkdir(model_params["tf_save_feature_path"]+'_EV')
        cmkdir(model_params["tf_save_feature_path"]+'_type')
        ...
        # extact feature
        model.eval()
        p_bar = tqdm(total=total_iter * (3 * model_params['extract_epochs'] + 4), desc='saving', ncols=100, mininterval=1, maxinterval=10, miniters=1)
