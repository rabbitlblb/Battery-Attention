...
class Label:
    ...
    def loss(self, batch, mean_pred, is_mse=True, reduce=True):
        ...
        for i in batch[1][self.label]:
            ...
            if cn in self.min_mileage.keys():
                norm_label = (i - self.min_mileage[cn]) / (self.max_mileage[cn] - self.min_mileage[cn])
            else:
                norm_label = (i - self.default_min_mileage) / (self.default_max_mileage - self.default_min_mileage)
