...
class Normalizer:
    def __init__(self, dfs=None, variable_length=False, debug = 0):
        self.max_norm = 0
        self.min_norm = 0
        self.std = 0
        self.mean = 0
...
def get_dataset(self, data_pre, axiv=False):
    battery, dataset = [], []
    dataloaders = defaultdict(list)
    badict = defaultdict(list)
    for i in range(self.args.battery_num):
        if i < len(data_pre):
            battery.append(PreprocessNormalizer(data_pre[i], normalizer_fn=self.normalizer[i].norm_func))
            for df,metadata in data_pre[i]:
                dataset += [(self.normalizer[i].norm_func(df), metadata)]
                badict[metadata['car']].append((self.normalizer[i].norm_func(df), metadata))
        else:
            battery.append([])
        
    for k, v in badict.items():
        dl = DataLoader(dataset=v, batch_size=self.args.batch_size, shuffle=True,
        ...
        if axiv:
            dataloaders[k[0]].append(dl)
        else:
            dataloaders[k]=dl
        
    return battery, dataset, list(dataloaders.values())

def idx2cn(idx):
    idx = int(idx)
    ret = torch.div(idx, 1000, rounding_mode='trunc')
    if ret > 0:
        ret -= 1
    return ret
