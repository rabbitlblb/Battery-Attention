...
from model import dynamic_vae, distinct_model, transformer_with_BA, transformer_with_CA
...
class Train_adaption:
    ...
    def main(self):
        ...
        data_pre = [dataset.load(self.args.train_path, ind_ood_car_dict_path=f'../five_fold_utils/ind_odd_dict{i}.npz.npy', fold_num=self.fold_num, sample_rate=self.args.sample_rate, unknown = self.args.unknown) for i in np.arange(1, self.args.battery_num+1)]
        ...
        self.normalizer = [Normalizer(dfs=[dpt[i][0] for i in range(len(dpt))],
                                     variable_length=self.args.variable_length, debug=dpt[0][1]['car'][0]) for dpt in data_pre_all]
        ...
        if self.args.model_type == "transformer_with_BA":
            model = to_var(transformer_with_BA.DynamicVAE(**params)).float()
        elif self.args.model_type == "DyAD":
            model = to_var(dynamic_vae.DynamicVAE(**params)).float()
        elif self.args.model_type == "TF":
            model = to_var(transformer_with_BA.DynamicVAE(**params)).float()
        elif self.args.model_type == "transformer_with_CA":
            model = to_var(transformer_with_CA.DynamicVAE(**params)).float()
        elif self.args.model_type == "IndependentDyAD":
            model = to_var(distinct_model.DynamicVAE(dynamic_vae, **params)).float()
        elif self.args.model_type == "IndependentTF":
            model = to_var(distinct_model.DynamicVAE(transformer_with_BA, **params)).float()
        else:
            model = None
        ...
        try:
            ...
            self.label_data = tasks.Label(column_name="mileage", training_set=self.train)
            ...
            while self.current_epoch <= self.args.epochs:
                ...
                loss_info = {'train_mean_loss': self.train_total_loss / (1 + self.train_iteration), 'train_nll_loss': self.train_total_nll / (1 + self.train_iteration),
                            ...
                            'val_mean_loss': -1, 'val_nll_loss': -1,
                            "val_label_loss": -1, "val_kl_loss": -1,
                            "AUCS": [-1] * 6}
        ...
        self.loss_visual()
        dataloaders = []
        for x in self.data_loader_battery_train:
            dataloaders += x
        p_bar = tqdm(total=sum([len(dl) for dl in dataloaders])  * (self.args.extract_epochs + 2), desc='saving', ncols=100, mininterval=1, maxinterval=10, miniters=1)
        norm_params = None
        extract(vars(self.args), self.step, self.train, dataloaders, model, self.data_task, self.args.feature_path, self.args.tf_feature_path, p_bar, self.args.noise_scale,
                self.args.variable_length, norm_params = norm_params, extract_epochs=self.args.extract_epochs)
        norm_params_path = os.path.join(self.args.current_model_path, "norm_params.pkl")
